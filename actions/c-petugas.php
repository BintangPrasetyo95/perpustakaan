    ...
    if (empty($username) || $username == '' || empty($email) || $email == '' || empty($nama_lengkap) || $nama_lengkap == '' || empty($alamat) || $alamat == '') {
        ...
        if ($pw == $pw_konf) {
            ...
            if (rows($cek_username) > 0 || rows($cek_email) > 0 || rows($cek_nama) > 0) {
                ...
                $query = query("INSERT INTO user SET nama_lengkap='$nama_lengkap', username='$username', email='$email', alamat='$alamat', password='$password', role='petugas' ");
            }
    ...
    if (empty($username) || $username == '' || empty($email) || $email == '' || empty($nama_lengkap) || $nama_lengkap == '' || empty($alamat) || $alamat == '') {
        ...
        if (!empty($_POST['pw']) || !empty($_POST['pw_konf'])) {
            ...
            if ($pw == $pw_konf) {
                ...
                if ($query) {
                    echo "<script>alert('Edit Petugas berhasil')</script>";
                } else {
                    echo "<script>alert('Edit Petugas gagal')</script>";
                }
            } else {
                echo "<script>alert('Password tidak sama')</script>";
            }
...
if (isset($_POST['hapus'])) {
    ...
    if ($query) {
        echo "<script>alert('Hapus Petugas berhasil')</script>";
    } else {
        echo "<script>alert('Hapus Petugas gagal')</script>";
    }
}
